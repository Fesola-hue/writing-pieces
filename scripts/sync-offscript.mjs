...
const topicMap = {
  ...
  '006': ['Nigeria', 'Systems', 'Money', 'Everyday life'],
  '007': ['Nigeria', 'Technology', 'Work', 'Everyday life', 'Systems']
};
...
async function main() {
  ...
  for (const entry of entries) {
    ...
    discovered.push({
      title: article.headline || entry.name || prior.title || `Issue ${issueNumber}`,
  ...
  const preserved = previous.issues.filter((issue) => !liveNumbers.has(issue.issueNumber));
